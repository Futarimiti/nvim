...
let s:all_chars_map = {}
      \->extend(s:visible_chars_map)
      \->extend(s:invisible_chars_map)

let s:visible_chars_regex = '[' .. keys(s:visible_chars_map)->join('') .. ']'
let s:invisible_chars_regex = '[' .. keys(s:invisible_chars_map)->join('') .. ']'

function! s:HighlightTrolling()
...
augroup TrollHighlight
  autocmd!
  autocmd BufEnter,WinEnter * call <SID>HighlightTrolling()
augroup END

command! -range=% TrollStop call <SID>TrollStop(<line1>, <line2>)

highlight link TrollStopper Normal
highlight Conceal NONE
highlight link Conceal Error

let s:troll_on = 0

function TrollToggle() abort
  if s:troll_on
    highlight! clear TrollStopper
  else
    highlight! link TrollStopper Error
  endif
  let s:troll_on = !s:troll_on
endfunction
command -bar TrollToggle call TrollToggle()
nnoremap <LocalLeader>T <Cmd>TrollToggle<CR>

" neovim/after/plugin/suspicious.vim highlight! TrollStopper ctermbg=NONE guibg=NONE
