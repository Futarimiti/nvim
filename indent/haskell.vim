...
function! s:GetHaskellIndent()
  ...
  if l:prevline =~ '\C\<let\>\s\+[^=]\+=\s*$'
    return match(l:prevline, '\C\<let\>') + 4 + shiftwidth()
  endif

  " let x = 1 in
  " >>>>x
  if l:prevline =~ '\C\<let\>.\{-}\<in\>\s*$' && l:line !~ '\C^\s*\<in\>'
    return match(l:prevline, '\C\<let\>') + 4
  endif
  ...
  if l:prevline =~ '\C\<case\>.\+\<of\>\s*$'
    if get(g:,'haskell_indent_case_alternative', 0)
      ...
      return match(l:prevline, '\C\<case\>') + 2
    endif
  endif
...
let &l:indentexpr = expand('<SID>') .. 'GetHaskellIndent()'
